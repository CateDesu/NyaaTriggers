...
def _word(pattern: str, text: str):
    # Use alphanumeric boundaries so underscores also separate category words.
    ...
    if top.lower() == 'sharing channel' and len(parts) > 1:
        ...
        if 'ultimate' in category:
            ...
            if 'fru' in path.lower() or 'futures rewritten' in path.lower():
                return 'FRU'
            # Match whole tags so STOP and DESKTOP cannot match TOP.
...
def extract_ids(regex: str) -> list[tuple[str, str]]:
    ...
    # Colon syntax can include a skip group or an ACT type name before the hex type.
...
def main() -> None:
    ...
    for t in existing:
        if not isinstance(t, dict):
            continue
        # Match the runtime's coercion of numeric IDs to strings.
